...
from EMInfraAPI.CertRequester import CertRequester
from EMInfraAPI.JWTRequester import SingletonJWTRequester, JWTRequester
...
class RequesterFactory:
    ...
    def create_requester(cls, settings: Dict = None, auth_type: str = '', env: str = '',
        ...
        if auth_info['type'] == 'JWT':
            ...
            requester = JWTRequester(private_key_path=auth_info['key_path'], client_id=auth_info['client_id'],
                                     first_part_url=first_part_url)
            requester.environment = env
            return requester

        if auth_info['type'] == 'cert':
            requester = CertRequester(cert_path=auth_info['cert_path'],
